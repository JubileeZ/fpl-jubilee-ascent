...
**Status**: Archived (2026/27 preseason). Active sibling calendar of Canonical Preseason Chip Path (same Prior-Season Dual-Vector Seed; does not replace GW1–6 BB1+WC4)
...
**Artifacts**:
- [Summary](first_half_summary.csv)
- [Weeks](first_half_weeks.csv)
- [Squads](first_half_squads.csv)
- [Transfers](first_half_transfers.csv)
- [Select-11 plan (WC4)](first_half_select_11.csv)
- [FH search](fh_week_search.csv)
- [Projections](gw1-19_projections.csv)
- [Dual-Vector seed](prior_season_dual_vector_seed.csv)
- [Canonical S1 re-score](canonical_s1_dual_vector_rescore.csv)
...
```text
Full redo docs/archive/gw1-19-first-half-chip-path/gw1-19-first-half-chip-path.md

1. Re-read CONTEXT.md First-Half Chip Path + Prior-Season Dual-Vector Seed.
2. uv run python docs/archive/gw1-19-first-half-chip-path/run_all.py
3. uv run python docs/archive/gw1-19-first-half-chip-path/export_select_11.py
...
6. Select 11: `export_select_11.py` applies recorded FTs at each GW, then max-xP legal XI (1 GKP + DEF 3–5 / MID 2–5 / FWD 1–3). `first_half_weeks.csv` scores post-WC weeks on the end-state 15; the plan CSV uses the 15 after FTs through that GW. Deltas up to 1.92 xP (GW6).
...
Companion: `first_half_select_11.csv`. GW1 = all 15. Other GWs = legal 11. Captain in **bold**. `week_xp` here is the operational XI; `published_week_xp` is `first_half_weeks.csv` (end-state 15 after WC).
...
- Greedy FTs reverse (Sarr/Tavernier) — rest-of-horizon approx noise; treat log as net moves.
- Select-11 `week_xp` on FT-timed 15 ≠ `first_half_weeks.csv` after GW4 (max |Δ| 1.92 at GW6).
