...
def rescore_canonical_s1(pool: pd.DataFrame) -> dict:
    sim = pd.read_csv(ROOT / "docs/archive/gw1-6-preseason-pipeline/03-gw1-6-chip-wc4-squads/gw1-6_wc4_simulation.csv")
    ...
    for gw, squad, bb in [(1, pre, True), (2, pre, False), (3, pre, False), (4, post, False), (5, post, False), (6, post, False)]:
        ...
        caps[gw] = cap
    summary = pd.read_csv(
        ROOT / "docs/archive/gw1-6-preseason-pipeline/03-gw1-6-chip-wc4-squads/gw1-6_wc4_summary.csv"
    )
