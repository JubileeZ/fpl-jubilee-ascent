...
- **RQI standardization & research recalibration (3 Aug):** Grilling session finalized canonical Rotation Quality Index (RQI) formula ($0.25 S_{\text{fdr}} + 0.25 S_{\text{corr}} + 0.15 S_{\text{easy}} + 0.25 S_{\text{xp\_gain}} + 0.10 S_{\text{cost}}$). Added domain terms to `CONTEXT.md` and recorded ADR `docs/adr/0013-bottom-up-calibrated-component-metrics-and-rqi.md`. Recalibrated `docs/research/gkp-fixture-rotation/run_gkp_rotation_analysis.py`, `gkp_rotation_matrix.csv` (252 records), and `gkp-fixture-rotation.md`. Top GW1–6 rotation pairs: Wilson (£4.5m) + Sels (£5.0m) at RQI 82.66 / 100; Verbruggen (£4.5m) + Roefs (£5.0m) at RQI 78.16 / 100 (top xP points gain: +2.25 xP).
