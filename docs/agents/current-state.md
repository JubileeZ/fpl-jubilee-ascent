...
- **RQI standardization & research recalibration (3 Aug):** Grilling session finalized Points-Heavy Rotation Quality Index (RQI) formula ($0.40 S_{\text{tot\_xp}} + 0.20 S_{\text{fdr}} + 0.20 S_{\text{corr}} + 0.10 S_{\text{easy}} + 0.10 S_{\text{cost}}$) based on Total Rotated Points ($\text{tot\_rot\_xp}$) rather than relative gain ($\Delta xP$), suppressing unproven promoted keeper distortions. Added `⚠️ Promoted Proxy` tags for Wilson/COV & Butland/HUL. Recalibrated `docs/research/gkp-fixture-rotation/run_gkp_rotation_analysis.py`, `gkp_rotation_matrix.csv` (252 records), and `gkp-fixture-rotation.md`. Top PL-Proven rotation pairs: Verbruggen (£4.5m) + Lammens (£5.0m) at RQI 81.14 / 100 (GW1–6) & 77.12 / 100 (Full Season); Verbruggen (£4.5m) + Roefs (£5.0m) at RQI 77.10 / 100 (top points output: 23.54 $xP$). Top PL-Proven £9.0m pair: Verbruggen (£4.5m) + Kinsky (£4.5m) at RQI 67.99 / 100.

- **GKP fixture rotation & FDR correlation study (3 Aug):** Evaluated all 428 starter GKP pairs (£4.0m–£5.5m) with combined cost ≤ £9.5m across GW1–6, GW1–10, GW1–19, and GW1–38. Generated `data/research/gkp-fixture-rotation/gkp_rotation_matrix.csv` and `docs/research/gkp-fixture-rotation/gkp-fixture-rotation.md`.
