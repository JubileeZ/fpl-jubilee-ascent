...
- **Constrained WC4 sub-reports (13 Aug):** Split SUN bridge out of parent rotation note. Added unrestricted overall bridge. Parent `def-fixture-rotation.md` §1.4 is a hub. Children `wc4-sun-bridge.md` / `wc4-overall-bridge.md` refresh via `--sun-bridge-only` / `--overall-bridge-only`. Overall #1 `LIV-MCI-MUN-MUN-NFO` (0 SUN). SUN pick still `LIV-MCI-MUN-NFO-SUN`. Tests green.
