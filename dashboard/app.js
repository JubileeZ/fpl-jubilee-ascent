...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function setupTeamSelect() {
    if (!teamSelect || !window.mountClubMultiSelect) return;
    teamClubFilter = window.mountClubMultiSelect(teamSelect, {
      emptyLabel: "All clubs",
      onChange: renderTable,
    });
    teamClubFilter.rebuild(Array.from(new Set(allPlayers.map((p) => p.team))).sort());
  }
  ...
  function renderTable() {
    ...
    let filtered = allPlayers.filter(p => {
      ...
      if (teamClubFilter && !teamClubFilter.allows(p.team)) return false;
