...
def test_load_champion_trust_reads_bias_walkforward_and_provisional(tmp_path: Path) -> None:
    ...
    selection.write_text(
        '{"schema_version":1,"champion":"calibrated_matchup_hybrid","candidates":[],"promotion_status":"provisional"}',
        encoding="utf-8",
    )
    bias = tmp_path / "bias.csv"
    bias.write_text(
        "model,evaluation_season,gw_start,gw_end,signed_bias,mae\n"
        "calibrated_matchup_hybrid,2025-26,1,38,0.15,1.07\n"
        "calibrated_matchup_hybrid,2026-27,1,3,0.21,1.40\n",
        encoding="utf-8",
    )
    ...
    assert payload["champion"] == "calibrated_matchup_hybrid"
